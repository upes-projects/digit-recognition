...
struct mnist * getMnistData(char *str, long start, long end){
	FILE *fptr;
	...
	int *ans = (int *) malloc( (end - start + 1)* sizeof(int));
	...
	struct mnist * mnt = (struct mnist *) malloc(sizeof(struct mnist));
	mnt->sta = sta;
	mnt->ans = ans;
